Здравствуйте!


Пользователь <?php echo $author->receivingUser->profiles->fullName() ?> отклонил Ваше предложение по перевозке груза на Вашем транспорте "<?php echo $author->vehicle->shortName(); ?>":

Если у Вас возникли какие-либо вопросы по нашему сервису, обратитесь к администрации сайта www.frahts.com: support@frahts.com



С уважением,

Администрация сайта <a href="frahts.com">www.frahts.com</a>