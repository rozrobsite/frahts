<div class="mainNav">
	<div class="user">
		<a href="javascript:void(0);" class="leftUserDrop">
			<?php $avatar = (isset($this->user) && isset($this->user->profiles) && isset($this->user->profiles->avatar)) ? '/' . Yii::app()->params['files']['avatars'] . $this->user->profiles->avatar : '/images/' . Yii::app()->params['images']['defaultAvatar']; ?>
			<img src="<?php echo $avatar . '?r=' . rand(0, 10000); ?>" width="72" height="70" alt="Аватар пользователя" />
		</a>
		<span>
			<?php
		...
		<ul class="leftUser">
			...
			<?php if ($this->user->profiles): ?>
				...
				<?php if ($this->user->profiles->user_type_id == 2 || $this->user->profiles->user_type_id == 3 ): ?>
					<li><a href="/goods/search" title="Личный груз пользователя" class="sBox">Мои грузы</a></li>
				<?php endif; ?>
			<?php endif; ?>
<!--			<li><a href="#" title="Сообщения" class="sMessages">Сообщения</a></li>
	...
	<ul class="nav">
		<?php if ($this->user->profiles): ?>
			...
			<?php endif; ?>
			<?php if ($this->user->profiles->user_type_id == 1 || $this->user->profiles->user_type_id == 3 ): ?>
				<li><a href="/vehicle/search" title="Поиск подходящего груза для перевозки" class="tipW" original-title="Поиск подходящего груза для перевозки"><img src="/images/icons/mainnav/goods.png" alt="Поиск транспорта для грузоперевозок" /><span>Поиск грузов</span></a></li>
			<?php endif; ?>
		<?php endif; ?>
