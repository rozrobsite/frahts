...
class UserController extends JokerController
{
    ...
    public function __construct($id, $module = null)
    {
        ...
        if (!$this->jokerUser)
            $this->redirect('/joker/main/login');
    }
    ...
    public function actionIndex()
    {
        ...
        if (isset($_POST['JokerProfiles'])) {
            ...
            if ($this->jokerUser->profiles->save()) {
                ...
                Yii::app()->user->setFlash('_error', 'Ваши данные не были сохранены. Проверьте введенные данные и попробуйте еще раз.');
            }
        }
        ...
        if (isset($_POST['JokerUsers'])) {
            ...
            if ($model->validate()) {
                ...
                if ($this->jokerUser->update()) {
                    ...
                    Yii::app()->user->setFlash('_success', 'Ваш элетронный адрес (e-mail) изменен успешно.');
                }
                else {
                    Yii::app()->user->setFlash('_error', 'Ваш элетронный адрес (e-mail) не был изменен. Проверьте введенные данные и попробуйте еще раз.');
                }
            }
            else {
                Yii::app()->user->setFlash('_error', 'Ваш элетронный адрес (e-mail) не был изменен. Проверьте введенные данные и попробуйте еще раз.');
        ...
        if (isset($_POST['JokerUsers'])) {
            ...
            if ($this->jokerUser->validate()) {
                if ($this->jokerUser->update()) {
                    ...
                    Yii::app()->user->setFlash('_error', 'Ваш пароль не был изменен. Проверьте введенные данные и попробуйте еще раз.');
                }
            }
        }
    ...
    public function actionOrganization()
    {
        ...
        if (isset($_POST['JokerOrganizations'])) {
            ...
            if ($this->jokerUser->organizations->save()) {
                ...
                Yii::app()->user->setFlash('_error', 'Ваши данные об организации не были сохранены. Проверьте введенные данные и попробуйте еще раз');
            }
    ...
    public function actionEmployeeUpdate()
    {
        ...
        if (isset($_POST['id']) && $_POST['id']) {
            $model = JokerEmployee::model()->findByPk((int)$_POST['id']);
            $model->prepare($_POST);
    ...
    public function actionEmployeeDelete()
    {
        ...
        if (isset($_POST['id']) && $_POST['id']) {
            $deleted = JokerEmployee::model()->deleteAllByAttributes(array('id' => (int)$_POST['id']));
