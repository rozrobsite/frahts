...
class EAjaxUpload extends CWidget
{
	...
	public $css=null;
		...
		$config = array(
			...
			'multiple' => false,
			'request'  => array(
				'endpoint' => $this->config['action']
			),
			'validation' => array(
				'allowedExtensions' => $this->config['allowedExtensions'],
				'minSizeLimit'      => !empty($this->config['minSizeLimit']) ? $this->config['minSizeLimit'] : '',
				'sizeLimit'         => $this->config['sizeLimit'],
				'acceptFiles'       => !empty($this->config['acceptFiles']) ? $this->config['acceptFiles'] : ''
			),
		);
		foreach (array('action', 'allowedExtensions', 'minSizeLimit', 'sizeLimit', 'acceptFiles') as $key) {
			unset($this->config[$key]);
		}
		$config = array_merge($config, $this->config);
		$config['params']=$postParams;
		$config = CJavaScript::encode($config);
                Yii::app()->getClientScript()->registerScript("FileUploader_".$this->id, "var FileUploader_".$this->id." = new qq.FineUploader($config); ",CClientScript::POS_LOAD);
	}
