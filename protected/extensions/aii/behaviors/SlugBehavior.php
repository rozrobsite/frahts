...
//				$this->getOwner()->{$this->slugAttribute} = $this->translateSlug($this->getOwner()->{$this->sourceAttribute});
	...
	public function afterSave($event)
	{
		...
		if ($this->getOwner()->isNewRecord)
		{
			...
			if (isset($this->nameMake))
			{
				$this->getOwner()->nameMake = $this->convertToSlug($this->getOwner()->make->name . ' ' . $this->getOwner()->model->name);
			}
	...
	{
		$source = str_replace(array_keys($this->replaceList),
				array_values($this->replaceList), $source);
		$source = htmlentities($source, ENT_COMPAT, 'UTF-8');
		$source = preg_replace(array_keys($this->cleanList),
				array_values($this->cleanList), $source);
	...
	protected function translateSlug($source)
	{
		...
		if (isset($translated)) return $this->convertToSlug($translated);
