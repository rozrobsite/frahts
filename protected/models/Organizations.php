...
class Organizations extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('form_tax', 'length', 'max'=>10),
			array('name_org', 'length', 'max'=>25),
			array('license', 'length', 'max'=>40),
			array('user_id', 'length', 'max'=>11),
			array('address', 'length', 'max'=>42),
			array('account_number', 'length', 'max'=>20),
			array('city', 'length', 'max'=>25),
			array('mfo', 'length', 'max'=>8),
			array('inn', 'length', 'max'=>12),
			array('certificate, edrpou', 'length', 'max'=>8),
			array('bank, phone', 'length', 'max'=>32),
			array('account_number, mfo, inn, certificate, edrpou', 'match', 'pattern'=>'/^[0-9]+$/u', 'message'=>'Должны быть только цифры.'),
